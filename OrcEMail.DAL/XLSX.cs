        ...
        public void ExportarParaExcel<T>(List<T> _Lentidade, string PathArquivo)
        {
            ...
            foreach (var p in props)
            {
                ...
                if (info.PropertyType.Name.Equals("String") || info.PropertyType.Name.Equals("Char") || info.PropertyType.Name.Equals("Guid") || info.PropertyType.Name.Equals("Object"))
                    TipoColuna[((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "string";
                else if ((info.PropertyType.Name.Equals("Int32")) || (info.PropertyType.Name.Equals("Int64")))
                    TipoColuna[((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "Int";
                    ...
                    TipoColuna[((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "decimal";
                else if (info.PropertyType.Name.Equals("DateTime"))
                    TipoColuna[((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "DateTime";
                else if (info.PropertyType.Name.Equals("bool"))
                    TipoColuna[((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = "bool";


                for (int i = 0; i < _Lentidade.Count; i++)
                {
                    T _entidade = _Lentidade[i];
                    arrHeader[i, ((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)Obj.GetValue(0)).Ordem] = Convert.ToString(p.GetValue(_entidade, null));
                }
            }
        ...
        public List<T> RetornaEntidade<T>(string PathArquivo)
        {
            ...
            foreach (PropertyInfo pi in Activator.CreateInstance(typeof(T)).GetType().GetProperties())
            {
                foreach (object objAux in tipo.GetProperty(pi.Name).GetCustomAttributes(true))
                {
                    ...
                    {
                        if (MaxIndice < ((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)objAux).Ordem)
                            MaxIndice = ((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)objAux).Ordem;
                        ObjIndices.Add(pi.Name, ((OrcEMail.DTO.XLSX_Coluna.AtributoXLS_Coluna)objAux).Ordem);
                    }
                    else if (objAux.GetType().Name == "AtributoXLS")
                    {
                        InicioLinha = ((OrcEMail.DTO.XLSX_Planilha.AtributoXLS)objAux).Linha + 1;
                        InicioColuna = ((OrcEMail.DTO.XLSX_Planilha.AtributoXLS)objAux).Coluna;
                    }
                }
